...
// error from the failed call. A secret checkbox picks the mode at creation only
// (locked on edit — flipping could reveal a write-only secret): secret =
// write-only value field (blank keeps, checkbox clears); regular = a plaintext
// value field, prefilled + editable. The write-only rules are enforced in Rust;
// success closes and asks the designer's page to refetch the registry.
export default function VariableModal({
    ...
    onClose: () => void;
    // a variable was written or deleted — the toolbox list and the catalog
    // entries derived from it are stale until the page refetches
    onSaved: () => void;
}) {
    ...
    const onDelete = async () => {
        ...
        setDeleting(true);
        try {
            await call("delete_registry_entry", { id: editing.id });
        } catch (err) {
            ...
            >
                <form
                    action={async (formData) => {
                        setError(null);
                        try {
                            await call("save_variable", {
                                id: editing?.id ?? null,
                                name: String(formData.get("name") ?? "").trim(),
                                value: String(formData.get("value") ?? "").trim(),
                                clearValue: formData.get("clearValue") === "on",
                                // ignored on edit — Rust reads the stored mode
