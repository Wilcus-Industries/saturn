...
LLM loop over the user's OpenRouter key. Inputs: flow in, prompt (value). Config: system (textarea), model (OpenRouter id like "openai/gpt-4o-mini"), output ("text" | "plan"), tools, skills. tools/skills are JSON string arrays as strings: tools entries are "<entryId>:<toolName>" (the mcp node key without the "mcp:" prefix), skills entries are skill entry uuids. The agent may call granted tools itself. Outputs: result (final text), plan (validated task-list JSON when output=plan, else empty).
