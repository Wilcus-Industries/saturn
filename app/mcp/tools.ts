...
## Built-in nodes
- schedule: scheduled entry point (config.cron, see above). if: routes flow to true/false comparing the left ("l") vs right ("r") operand value ports (config.operator). loop: runs body once per item of the JSON array on items, then done; item carries the current element. and/or/not: boolean values. string: emits config.value verbatim on its "out" value output. number: emits config.value coerced to a number ("out"). print: logs its "message" input — the connected port overrides the config.message literal (overriddenBy). concat: joins the "a" and "b" value inputs into one string on "out". extract: pulls a field out of a JSON value via config.path, dot-separated with numeric array indices ("data.results.0.price"). await: join barrier for parallel branches — continues when ALL incoming flow edges arrive; results = JSON array of its values edges (multi port), in edge order. model: emits config.model (an OpenRouter model id) on its "model" value output — connect it to an agent's model input. log-append: appends one message to a persistent per-user chat log — the "key" input port (or config.key literal, overriddenBy) names the log (e.g. a Discord channel id wired from an extract on the event payload), "message" (port or config.message) is the text, config.role is "user" or "bot"; each (user, key) log keeps only its newest 200 messages, and an empty message is skipped without failing the run. log-read: a FLOW node, not a pure value — it must run (flow in) before its output is readable; loads the last config.limit messages (default 20, max 200) for its key (same port-or-literal pairing) and emits them oldest-first on its "history" value output as one transcript string, one "[M/D HH:MM] role: text" line per message (UTC, each message truncated to 1000 chars; empty log ⇒ empty string). Together they give bot workflows conversation memory: event → log-append the incoming message (role user) → log-read → build the prompt (concat history + instructions) → agent → log-append the agent result (role bot) → send.
