...
// one normalized inbound-event subscription: everything the gateway needs to
// hold a Discord connection and route a mention. guild/channel are optional
// filters.
...
export async function getEventSubscriptions(): Promise<EventSubscription[]> {
    ...
    for (const wf of rows) {
        for (const node of wf.graph.nodes) {
            ...
            if (!event) continue;
            // skip nodes missing required config — the gateway can't connect
            // without a bot token
...
// push-invalidation seam: workflow mutations poke the gateway to re-poll
// subscriptions without waiting out its 60s interval. A plain callback keeps
// `ws` (and the whole gateway module) out of server-action bundles.
let subscriptionsListener: (() => void) | null = null;

export function onSubscriptionsChanged(fn: (() => void) | null) {
    subscriptionsListener = fn;
}

// fire-and-forget; no-op when the gateway isn't running
