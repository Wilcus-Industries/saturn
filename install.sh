...
if [[ "${WRITE_ENV}" -eq 1 ]]; then
  ...
  umask 177
  # node-postgres v8.16+ warns on sslmode=require (v9 will weaken its semantics) —
  # write the verify-full form the app runs under today. psql calls in this script
  # keep the original: libpq's verify-full needs a root-cert setup we don't assume.
  cat > .env.local <<EOF
# generated by install.sh $(date -u +%Y-%m-%dT%H:%M:%SZ) — SELF_HOSTED single-owner mode
SELF_HOSTED=1
DATABASE_URL=${DB_URL/sslmode=require/sslmode=verify-full}
